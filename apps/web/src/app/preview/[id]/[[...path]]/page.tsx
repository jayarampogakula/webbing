...
export default async function ProjectPreviewPage({ params }: { params: { id: string; path?: string[] } }) {
  try {
    ...
    const isInternalCheck = headers().get("x-internal-publish-check") === "true";
    if (!isInternalCheck) {
    ...
    if (isPrivacyPage || isTermsPage || isRefundPage) {
      ...
      let themeClass = "theme-startup";
      if (designStyle === "Gaming") themeClass = "theme-gaming";
      else if (designStyle === "Creator") themeClass = "theme-creator";
      else if (designStyle === "Luxury") themeClass = "theme-luxury";
      else if (designStyle === "Fitness") themeClass = "theme-fitness";
      else if (designStyle === "SaaS") themeClass = "theme-saas";
      ...
      return (
        <div className={`site-preview ${themeClass} ${bgClass} ${fontClass}`} style={{ minHeight: "100vh", position: "relative", overflowX: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          <header style={{ padding: "1.5rem 2rem", borderBottom: "1px solid rgba(255, 255, 255, 0.06)", display: "flex", justifyContent: "space-between", alignItems: "center", maxWidth: "1100px", margin: "0 auto", width: "100%" }}>
            <a href={`/preview/${project.id}`} style={{ color: "#fff", textDecoration: "none", fontSize: "1.25rem", fontWeight: 800 }}>
              {renderText(project.name)}
            </a>
            <a href={`/preview/${project.id}`} style={{ color: "#818cf8", textDecoration: "none", fontSize: "0.85rem", fontWeight: 600 }}>
              Back to Home
            </a>
          </header>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", color: "#9ca3af", fontSize: "0.85rem", flexWrap: "wrap", gap: "1rem" }}>
                    ...
                    <div style={{ display: "flex", gap: "1.5rem" }}>
                      {policies.privacyPolicyEnabled && (
                        <a href={`/preview/${project.id}/privacy-policy`} style={{ transition: "color 0.2s" }}>Privacy Policy</a>
                      )}
                      {policies.termsEnabled && (
                        <a href={`/preview/${project.id}/terms-of-service`} style={{ transition: "color 0.2s" }}>Terms of Service</a>
                      )}
                      {policies.refundPolicyEnabled && (
                        <a href={`/preview/${project.id}/refund-policy`} style={{ transition: "color 0.2s" }}>Refund Policy</a>
                      )}
                    </div>
                  </div>
                  {isFree && (
